...
from game_data import (
    ...
    RETIREMENT_DEATH_REASONS,
    TOWN_BODY_PARTS,
    TOWN_INJURIES,
    TOWN_JOB_OFFER_CHANCE,
...
class GameController:
    ...
    def _restore_payload(self, payload: object) -> bool:
        ...
        engine.base_skills.pop("salvaging", None)
        valid_equipment_slots = {"fighting_weapon", "defending_armor", "accessory_1", "accessory_2"}

        def is_legacy_salvaging_item(item):
            return bool(item) and (
                item.get("category") == "salvaging"
                or item.get("slot") == "salvaging_tool"
                or item.get("skill") == "salvaging"
            )

        engine.inventory = [
            item for item in engine.inventory
            if not is_legacy_salvaging_item(item)
        ]
        engine.equipment = {
            slot: item for slot, item in engine.equipment.items()
            if slot in valid_equipment_slots and not is_legacy_salvaging_item(item)
        }
        engine.relics_found = [relic for relic in engine.relics_found if relic in RELICS]
        self.engine = engine
    ...
    def _build_character_stats_rows(self):
        ...
        rows = []
        for skill in ("fighting", "defending", "magic", "stealth", "speech"):
